...
Este trabajo aborda el reto de identificar estados de dolor crónico a partir de volúmenes fMRI de rata mediante redes neuronales convolucionales.

El proyecto extiende el estudio previo de **Alan Macías**, quien empleó un modelo **VGG16 3D** para clasificar cinco estados de dolor, incorporando ahora un análisis más amplio y comparativo de métodos de interpretabilidad. Además del método base **Grad-CAM**, se integraron **Grad-CAM++**, **Respond-CAM** y **HiResCAM**, con el propósito de fortalecer la comprensión de las regiones cerebrales que contribuyen a la predicción del modelo.

Dado que algunos mapas de activación presentan señales fuera del cerebro, se añadió la métrica **coverage** para cuantificar la proporción anatómicamente válida de las activaciones (activaciones dentro del cerebro respecto al total).

Asimismo, se creó un escenario comparativo complementario a los cinco desarrollados por el autor original, siendo el escenario **CPH Female BL vs W1W7**, evaluado con **10 y 50 épocas**.

---

# Estado del proyecto

Después de realizar la comparativa entre los métodos de interpretabilidad, se obtuvo un mejor desempeño para **Grad-CAM++**.  
Aunque **Grad-CAM** y **HiResCAM** presentan las coberturas más altas, exhiben variabilidad notable entre sujetos (ratas macho y hembra), posicionando a **Grad-CAM++** como el método más estable para este tipo de datos y el modelo empleado.

Se sugiere complementar la evaluación mediante la inclusión de métricas adicionales que permitan abarcar de forma más integral los factores involucrados en la efectividad de los métodos de interpretabilidad.

---

*A continuación se abordan las consideraciones necesarias para replicar el presente trabajo, con el objetivo de robustecer el avance efectuado.*
...
subst R: "C:\Users\L03117947\Instituto Tecnologico y de Estudios Superiores de Monterrey\Luis Guillermo Hernández Rojas - rabies"
